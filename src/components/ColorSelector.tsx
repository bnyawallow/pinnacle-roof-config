...
const ColorSelector = ({ colors, selectedColor, onSelectColor }: ColorSelectorProps) => {
  ...
  const hasScrolling = colors.length > 4;

  return (
    <Card className={`bg-white/10 backdrop-blur-md overflow-hidden relative p-3 border border-white/20 ${hasScrolling ? 'h-full' : ''}`}>
      {hasScrolling && (
      ...
      <div ref={hasScrolling ? scrollRef : null} className={`space-y-1 px-1 ${hasScrolling ? 'overflow-y-auto pb-10 pt-10 scrollable h-full' : ''}`}>
        {colors.map((color) => (
          <button
            key={color.id}
            onClick={() => { onSelectColor(color.id); }}
            className="relative w-full flex flex-col items-center justify-center p-3 gap-2 rounded-xl transition-all duration-200 hover:bg-[#fe6b35]/5"
          >
            <div
